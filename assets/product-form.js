...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getSelectedOptions() {
    return [...form.querySelectorAll(".product-options input:checked")].map((i) => i.value).join(" / ");
  }

  function updateVariantId() {
    const selected = getSelectedOptions();

    const match = variants.find((v) => v.options.join(" / ") === selected);
  ...
  form.addEventListener("change", (e) => {
    if (e.target.matches(".product-options input")) {
      updateVariantId();
    }
  });
});
